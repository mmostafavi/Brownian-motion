...
def beadtracker(min_pixels, tau, delta, file1, file2):
    ...
    # finding all blobs in each picture
    blobs1 = Beadfinder(pic1, tau)
    blobs2 = Beadfinder(pic2, tau)

    # filter all blobs with more than
    # min_pixels
    beads1 = blobs1.getBeads(min_pixels)
    beads2 = blobs2.getBeads(min_pixels)

    # "found" stands for all result that
    # will be return in end.
    found = []

    # we use array bellow to don't
    # calculate a dot two more thab once
    conected_beads = [0]*len(beads2)

    for i in range(len(beads1)):
        # 20000 because in pixels can't
        # be more than or even near to
        # this number
        temp = [20000, beads2[0]]
...
# we use index function to
# construct three digits of
# right of frame00xxx
